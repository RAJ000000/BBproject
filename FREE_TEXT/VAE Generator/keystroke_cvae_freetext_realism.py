...
def load_data(csv_path):
    ...
    # Mild clipping to remove only extreme outliers
    clip_bounds = {}
    for col in feature_cols:
    ...
    # RobustScaler works better for skewed/heavy-tailed free-text timings
    scaler = RobustScaler()
    X_scaled = scaler.fit_transform(X).astype(np.float32)

    # Light clipping after robust scaling
...
class CVAE(nn.Module):
    def __init__(self, input_dim, cond_dim, latent_dim=16, dropout=0.03):
        super().__init__()
...
def build_model(input_dim, cond_dim, latent_dim=16, dropout=0.03):
    return CVAE(input_dim, cond_dim, latent_dim=latent_dim, dropout=dropout)


# =========================================================
# 6. LOSS
#    Huber reconstruction + KL annealing
# =========================================================
def loss_fn(recon, x, mu, logvar, beta=0.00005):
    recon_loss = nn.SmoothL1Loss()(recon, x)
...
def train(
    ...
    for epoch in range(epochs):
        # KL annealing
        beta = beta_max * min(1.0, (epoch + 1) / 15.0)
        ...
        print(f"Epoch {epoch + 1}/{epochs} | Beta {beta:.6f} | Train {avg_train:.6f} | Val {avg_val:.6f}")
...
# =========================================================
# 8. QUANTILE CALIBRATION
#    better than moment matching for skewed data
# =========================================================
def quantile_calibrate_feature(syn_vals, real_vals):
    syn_vals = np.asarray(syn_vals, dtype=np.float64)
    real_vals = np.asarray(real_vals, dtype=np.float64)

    syn_order = np.argsort(syn_vals)
    syn_sorted = syn_vals[syn_order]
    real_sorted = np.sort(real_vals)

    syn_quantiles = np.linspace(0, 1, len(syn_sorted))
    real_quantiles = np.linspace(0, 1, len(real_sorted))

    mapped = np.interp(syn_quantiles, real_quantiles, real_sorted)

    out = np.empty_like(mapped)
    out[np.argsort(syn_order)] = mapped
    return out


def quantile_calibrate_dataframe(synthetic_df, real_df, feature_cols, clip_bounds):
    calibrated = synthetic_df.copy()

    for col in feature_cols:
        syn_vals = calibrated[col].values
        real_vals = real_df[col].values

        mapped = quantile_calibrate_feature(syn_vals, real_vals)

        low, high = clip_bounds[col]
        mapped = np.clip(mapped, low, high)

        calibrated[col] = mapped
...
def generate_same_shape(
    ...
    if quantile_calibration:
        synthetic_df = quantile_calibrate_dataframe(synthetic_df, df, feature_cols, clip_bounds)

...
def prepare_train_test_data(real_file, vae_file, chatgpt_file, chunk_size=50, test_size=0.2, random_state=42):
    real_df = pd.read_csv(real_file, low_memory=False)
    vae_df = pd.read_csv(vae_file, low_memory=False)
    chatgpt_df = pd.read_csv(chatgpt_file, low_memory=False)

    real_df = clean_free_text_df(real_df)
    vae_df = clean_free_text_df(vae_df)
    chatgpt_df = clean_free_text_df(chatgpt_df)

    real_train_raw, real_test_raw = split_sessions(real_df, test_size=test_size, random_state=random_state)
    vae_train_raw, vae_test_raw = split_sessions(vae_df, test_size=test_size, random_state=random_state)
    chat_train_raw, chat_test_raw = split_sessions(chatgpt_df, test_size=test_size, random_state=random_state)
    ...
    vae_train_chunks = make_chunks(vae_train_raw, chunk_size=chunk_size)
    vae_test_chunks = make_chunks(vae_test_raw, chunk_size=chunk_size)

    chat_train_chunks = make_chunks(chat_train_raw, chunk_size=chunk_size)
    chat_test_chunks = make_chunks(chat_test_raw, chunk_size=chunk_size)

    real_train = aggregate_chunk_features(real_train_chunks)
    real_test = aggregate_chunk_features(real_test_chunks)

    vae_train = aggregate_chunk_features(vae_train_chunks)
    vae_test = aggregate_chunk_features(vae_test_chunks)

    chat_train = aggregate_chunk_features(chat_train_chunks)
    chat_test = aggregate_chunk_features(chat_test_chunks)
    ...
    train_df = pd.concat([real_train, vae_train, chat_train], ignore_index=True)
    test_df = pd.concat([real_test, vae_test, chat_test], ignore_index=True)

    train_df = train_df.sample(frac=1, random_state=random_state).reset_index(drop=True)
    test_df = test_df.sample(frac=1, random_state=random_state).reset_index(drop=True)

    print("Real raw shape:", real_df.shape)
    print("VAE raw shape:", vae_df.shape)
    print("ChatGPT raw shape:", chatgpt_df.shape)

    print("\nReal train chunks shape:", real_train.shape)
    print("Real test chunks shape:", real_test.shape)
    print("VAE train chunks shape:", vae_train.shape)
    print("VAE test chunks shape:", vae_test.shape)
    print("ChatGPT train chunks shape:", chat_train.shape)
    print("ChatGPT test chunks shape:", chat_test.shape)
...
def select_mean_difference(X_train, y_train, X_test, k=10):
    grouped_means = X_train.copy()
    grouped_means["label"] = y_train.values
    mean_by_class = grouped_means.groupby("label").mean(numeric_only=True)

    overall_diff = mean_by_class.max(axis=0) - mean_by_class.min(axis=0)
    overall_diff = overall_diff.sort_values(ascending=False)

    score_df = pd.DataFrame({"Feature": overall_diff.index, "Score": overall_diff.values})
    selected = score_df.head(k)["Feature"].tolist()
...
def select_sfm_lr(X_train, y_train, X_test, k=10):
    model = LogisticRegression(penalty="l1", solver="liblinear", max_iter=1000, multi_class="ovr")
...
def plot_feature_distribution_from_train_test(train_df, test_df, feature_name):
    ...
    vae = full_df[full_df["label"] == 1][feature_name]
    chatgpt = full_df[full_df["label"] == 2][feature_name]

    print("\nFeature:", feature_name)
    print("Real Mean:", real.mean())
    print("VAE Mean:", vae.mean())
    print("ChatGPT Mean:", chatgpt.mean())

    plt.figure(figsize=(6, 4))
    plt.hist(real, bins=20, alpha=0.5, label="Real")
    plt.hist(vae, bins=20, alpha=0.5, label="VAE")
    plt.hist(chatgpt, bins=20, alpha=0.5, label="ChatGPT")

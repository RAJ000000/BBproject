...
# ---------------------------------------------------
def load_data(real_file, vae_file, chatgpt_file):
    real_df = pd.read_csv(real_file)
    vae_df = pd.read_csv(vae_file)
    chatgpt_df = pd.read_csv(chatgpt_file)

    common_cols = [col for col in real_df.columns if col in vae_df.columns and col in chatgpt_df.columns]

    real_df = real_df[common_cols].copy()
    vae_df = vae_df[common_cols].copy()
    chatgpt_df = chatgpt_df[common_cols].copy()

    real_df["label"] = 0
    vae_df["label"] = 1
    chatgpt_df["label"] = 2

    df = pd.concat([real_df, vae_df, chatgpt_df], ignore_index=True)
...
def select_correlation(X_train, y_train, X_test, k=10):
    ...
    score_series = corr_matrix["label"].drop("label").abs().sort_values(ascending=False)
...
def select_mean_difference(X_train, y_train, X_test, k=10):
    ...
    overall_diff = pd.Series(0.0, index=X_train.columns)

    for col in X_train.columns:
        vals = class_means[col].values
        score = 0.0
        for i in range(len(vals)):
            for j in range(i + 1, len(vals)):
                score += abs(vals[i] - vals[j])
        overall_diff[col] = score

    overall_diff = overall_diff.sort_values(ascending=False)

    score_df = pd.DataFrame({
        "Feature": overall_diff.index,
        "Score": overall_diff.values
    })
...
# ---------------------------------------------------
# 13. Wrapper 4: SelectFromModel-style Logistic Regression
# ---------------------------------------------------
...
# ---------------------------------------------------
# 14. Wrapper 5: SelectFromModel-style Random Forest
# ---------------------------------------------------
...
# ---------------------------------------------------
# 19. Plot feature distribution
# ---------------------------------------------------
def plot_feature_distribution(df, feature_name):
    c0 = df[df["label"] == 0][feature_name]
    c1 = df[df["label"] == 1][feature_name]
    c2 = df[df["label"] == 2][feature_name]

    print("\nFeature:", feature_name)
    print("Real Mean:", c0.mean())
    print("VAE Mean:", c1.mean())
    print("ChatGPT Mean:", c2.mean())

    print("Real Std:", c0.std())
    print("VAE Std:", c1.std())
    print("ChatGPT Std:", c2.std())

    plt.figure(figsize=(6, 4))
    plt.hist(c0, bins=30, alpha=0.5, label="Real")
    plt.hist(c1, bins=30, alpha=0.5, label="VAE")
    plt.hist(c2, bins=30, alpha=0.5, label="ChatGPT")
